function [DDout,EEtot,ccond,ccorrel,en1,en2,en3,en4,ddelta]=gap_eq_solver(type,NF,t,mu,J,beta,V,kkx,kky,DDinput)
for s=1:length(DDinput)
    D=DDinput(s);
    for i=1:length(kkx)
        for j=1:length(kky)
            kx=kkx(i);
    ...
    DDout(s)=sum(sum(delta));
    EEtot(s)=sum(en1(en1<0))+sum(en2(en2<0))+sum(en3(en3<0))+sum(en4(en4<0));
    ccond(s)=sum(sum(cond));
    ccorrel{s}=correl;een1{s}=en1;een2{s}=en2;een3{s}=en3;een4{s}=en4;
    ddelta{s}=delta;
end
end