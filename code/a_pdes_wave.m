...
ii=5;
jj=50;
[Epk_u_test, Epk_d_test] = tripletEkJkMaker(kx, ky, ek, mu, Dp_self(ii), J_ran(ii),gpka);
[~, ~, correlp_test, ~] = tripletgapsolver(ek, mu, Dp_self(ii), J_ran(ii) * sin(kx) .* sin(ky), V_ran(jj), gpk, gpka);

[Eesk_u_test, Eesk_d_test] = singletEkJkMaker(kx, ky, ek, mu, Des_self(ii), J_ran(ii),geska);
[~, ~, correles_test, ~] = singletgapsolver(ek, mu, Des_self(ii), J_ran(ii) * sin(kx) .* sin(ky), V_ran(jj), gesk, geska);

[Edk_u_test, Edk_d_test] = singletEkJkMaker(kx, ky, ek, mu, Dd_self(ii), J_ran(ii), gdka);
[~, ~, correld_test, ~] = singletgapsolver(ek, mu, Dd_self(ii), J_ran(ii) * sin(kx) .* sin(ky), V_ran(jj), gdk, gdka);

figure
surf(reshape(Epk_u_test, [N,N]))
hold on
surf(reshape(Epk_d_test,[N,N]))
title('p-wave quasiparticle band structure')
% zlim([-1,0]);view(2)
figure;
surf(reshape(squeeze(abs(correlp_self(jj, ii, :,:))),[N,N]));view(2)
title('p-wave correlation')

figure;
surf(reshape(Eesk_u_test, [N,N]))
hold on
surf(reshape(Eesk_d_test,[N,N]))
title('es-wave quasiparticle band structure')
% zlim([-1,0]);view(2)
figure;
surf(reshape(squeeze(correles_self(jj, ii, :,:)),[N,N]));view(2)
title('es-wave correlation')

figure;
surf(reshape(Edk_u_test, [N,N]))
hold on
surf(reshape(Edk_d_test,[N,N]))
title('d-wave quasiparticle band structure')
% zlim([-1,0]);view(2)
figure;
surf(reshape(squeeze(correld_self(jj, ii, :,:)),[N,N]));view(2)
title('d-wave correlation')
%% function

function [Ek_u, Ek_d] = singletEkJkMaker(kx, ky, ek, mu, Delta, J, gka)
Jk = J * sin(kx) .* sin(ky);
...
function [Ek_u, Ek_d] = tripletEkJkMaker(kx, ky, ek, mu, Delta, J, gka)
Jk = J * sin(kx) .* sin(ky);
Ek_u = sqrt((ek - mu + Jk) .^ 2 + Delta ^2 * gka);
